...
def nearest2(request):
    ...
    if form1.is_valid():
        ...
        cursor.execute(
            "SELECT sname from metro_places where place =%s", [place])
...
def nearest3(request):
    ...
    if form2.is_valid():
        ...
        cursor = connection.cursor()
        cursor.execute(
            "SELECT sname from metro_stationinfo where pincode =%s", [pin])
...
def review2(request):
    ...
    if form1.is_valid():
        ...
        cursor = connection.cursor()
        cursor.execute(
            "SELECT sname,title,author,bodytext from metro_review where approved = 'Yes'")
        data = cursor.fetchall()
...
def review3(request):
    ...
    if form2.is_valid():
        ...
        cursor = connection.cursor()
        cursor.execute("INSERT INTO metro_review (sname,title,author,bodytext,timest,approved) VALUES (%s,%s,%s,%s,now(),'no')", [
                       sname, title, author, bodytext])
